...
public class LibreriaBean {
	...
	public List<SelectItem> getEditoriales() {
		List<SelectItem> lista = new ArrayList<SelectItem>();
		List<Editorial> listEditorial = gestionLibreria
				.consultarTodasEditoriales();
	...
	public List<SelectItem> getNacionalidades() {
		List<String> listaNacionalidades = gestionLibreria
				.consultarAutorNacionalidades();
		List<SelectItem> lista = new ArrayList<SelectItem>();
	...
	public List<Libro> consultarLibro() {
		if (libro.getIsbn().equals("")) {
			if (libro.getTitulo().equals(""))
				...
				listaLibros = this.gestionLibreria.consultarTituloLibro(libro
						.getTitulo());
		} else {
			listaLibros = new ArrayList<Libro>();
			Libro libro1 = this.gestionLibreria.consultarISBNLibro(libro
					.getIsbn());
			if (libro1 != null)
	...
	public List<Autor> consultarAutor() {
		if (autor.getNombre().equals("")) {
			if (autor.getNacionalidad().equals(""))
				...
				listaAutores = this.gestionLibreria.consultarNacAutor(autor
						.getNacionalidad());
		} else {
			listaAutores = this.gestionLibreria.consultarNomAutor(autor
					.getNombre());
	...
	public List<Editorial> consultarEditoriales() {
		if (editorial.getNif().equals("")) {
			if (editorial.getNombre().equals(""))
				listaEditoriales = this.gestionLibreria
						.consultarTodasEditoriales();
			else {
				listaEditoriales = this.gestionLibreria
						.consultarNomEditorial(editorial.getNombre());
			}
		} else {
			listaEditoriales = new ArrayList<Editorial>();
			Editorial editorial1 = this.gestionLibreria
					.consultarNifEditorial(editorial.getNif());
			if (editorial1 != null)
